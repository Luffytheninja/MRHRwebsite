            ...
            <div className="grid-2col" style={{ gap: "4rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
                <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", marginBottom: "1.25rem" }}>
                  ...
                  <span style={{ fontSize: "0.8125rem", color: "var(--clr-text-light)" }}>{newsItems[0].date}</span>
                </div>
                <h2 style={{ fontSize: "clamp(1.75rem, 3.5vw, 2.5rem)", fontFamily: "var(--font-serif)", fontWeight: "400", lineHeight: "1.2", marginBottom: "1.5rem" }}>
                  <Link href={`/news/${newsItems[0].slug}`} style={{ textDecoration: "none", color: "inherit" }}>
                    {newsItems[0].title}
                  </Link>
                </h2>
                <p style={{ fontSize: "1.1rem", color: "var(--clr-text-muted)", lineHeight: "1.75", marginBottom: "2rem" }}>
                  {newsItems[0].excerpt}
                </p>
                <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
                  <span style={{ fontSize: "0.875rem", color: "var(--clr-text-light)" }}>By {newsItems[0].author}</span>
                  <Link
                    href={`/news/${newsItems[0].slug}`}
                    style={{ fontSize: "0.8125rem", fontWeight: "700", color: "var(--clr-primary)", textDecoration: "none", letterSpacing: "0.06em" }}
                  >
                    READ ARTICLE →
                  </Link>
                </div>
          ...
          {newsItems.slice(1).map((item, i) => (
            <article key={i} style={{ display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", marginBottom: "1rem" }}>
                ...
                <time dateTime={item.date} style={{ fontSize: "0.75rem", color: "var(--clr-text-light)" }}>{item.date}</time>
              </div>
              <h3 style={{ fontSize: "1.25rem", fontFamily: "var(--font-serif)", fontWeight: "400", lineHeight: "1.4", marginBottom: "1rem", flex: 1 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid var(--clr-border)", paddingTop: "1rem", marginTop: "auto" }}>
                ...
                <Link
                  href={`/news/${item.slug}`}
                  style={{ fontSize: "0.75rem", fontWeight: "700", color: "var(--clr-primary)", textDecoration: "none", letterSpacing: "0.06em" }}
                >
                  READ MORE →
                </Link>
              </div>
      ...
      <section className="page-section--tinted">
        <div className="container">
          <div style={{ maxWidth: "580px", margin: "0 auto", textAlign: "center" }}>
            ...
            <h2 style={{ fontSize: "clamp(1.75rem, 3.5vw, 2.5rem)", fontFamily: "var(--font-serif)", fontWeight: "400", margin: "1rem 0 1.25rem" }}>
